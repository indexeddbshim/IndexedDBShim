...
- 'idb-partitioned-basic.tentative.sub.js' - Timing out (iframe)
...
- `interleaved-cursors-small.js` - One times out
- `interleaved-cursors-large.js` - One times out
...
Current IndexedDB (and domstringlist) test statuses (vmTimeout = 40000):
  'Pass': 1131, (including 4 domstringlist tests but avoiding exclusions)
  // Many failures or not run are from one structured cloning test and
  //  12+1+1 from missing commit API
  'Fail': 73,
  'Timeout': 11,
  'Not Run': 86,
  'Total tests': 1301 (including 4 domstringlist tests but avoiding exclusions)
...
const goodBad = {
    ...
    badFiles: [
        ...
        'idlharness.any.sharedworker2.js',
