  ...
  ['add', 'remove', 'has'].forEach(function (method) {
    ...
    obj[mainMethod] = function (type, listener) {
      ...
      if (captureOnly && method === 'add') {
        ...
        const rawOptions = typeof options === 'boolean' ? {
          capture: options
        } : options || {};
...
const readonlyProperties$7 = ['oldVersion', 'newVersion'];
...
/* eslint-disable unicorn/no-top-level-side-effects -- Would be good */
readonlyProperties$7.forEach(prop => {
...
const readonlyProperties$6 = ['source', 'transaction', 'readyState'];
...
function getCopyBytesHeldByBufferSource(O) {
  ...
  const arrayBuffer = ArrayBuffer.isView(O) ? O.buffer : O;
...
const readonlyProperties$5 = /** @type {const} */['lower', 'upper', 'lowerOpen', 'upperOpen'];
...
IDBTransaction.__createInstance = function (db, storeNames, mode, durability = 'default') {
  ...
  function IDBTransaction() {
    ...
    readonlyProperties$4.forEach(readonlyProp => {
...
const readonlyProperties$3 = ['objectStore', 'keyPath', 'multiEntry', 'unique'];
...
IDBIndex.__clone = function (index, store) {
  ...
  /** @type {const} */
  ['__pendingCreate', '__pendingDelete', '__deleted', '__originalName'].forEach(p => {
...
IDBIndex.__createIndex = function (store, index) {
  ...
  transaction.__addNonRequestToTransactionQueue(function createIndex(tx, args, success, failure) {
    ...
    const escapedIndexNameSQL = escapeIndexNameForSQL(indexName);
...
IDBIndex.prototype.__fetchIndexData = function (range, opType, nullDisallowed) {
  ...
  return /** @type {import('./IDBTransaction.js').IDBTransactionFull} */me.objectStore.transaction.__addToTransactionQueue(function (...args) {
    executeFetchIndexData(...fetchArgs,
    // @ts-expect-error It's ok
    ...args);
  }, undefined, me);
};
...
() {
  ...
  const {
    ...
    count,
    direction
  } = parseGetAllRecordsArgs(arguments);
...
const readonlyProperties$2 = ['keyPath', 'indexNames', 'transaction', 'autoIncrement'];
...
const readonlyProperties$1 = ['name', 'version', 'objectStoreNames'];
    ...
    function triggerSuccess(k, val, primKey) {
      if (advanceState) {
        if (me.__advanceCount && me.__advanceCount >= 2 && k !== undefined) {
          ...
          const {
            transaction
          } = /** @type {{transaction: import('./IDBTransaction.js').IDBTransactionFull}} */
          me.__store;
...
function collectAll(source, query, count, direction, mode) {
  const isIndexSource = instanceOf(source, IDBIndex);
  ...
  if (range !== undefined) {
    ...
    range.__lowerCached = range.lower !== undefined && encode$1(range.lower, multiEntryIndex);
    range.__upperCached = range.upper !== undefined && encode$1(range.upper, multiEntryIndex);
  }
  ...
  const keyColumnName = isIndexSource ? escapeIndexNameForSQLKeyColumn(indexSource.name) : 'key';
      ...
      if (state.__prefetchedData) {
        ...
        if (state.__prefetchedIndex < state.__prefetchedData.length) {
          ...
          function (k, val, primKey, encKey) {
            ...
            if (state.__unique && !state.__multiEntryIndex && encKey === encode$1(state.__key, state.__multiEntryIndex)) {
              runContinuationSafely(step);
              return;
            }
