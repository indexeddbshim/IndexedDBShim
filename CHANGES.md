...
- test: fix intermittent `IDBDatabase.close` timeout caused by a fake sysdb driver leaking past its own test into later spec files
- test: `clean-mocha` glob broadened from `D_sysdb-basepath*` to `D_sysdb-*` to catch all sysdb test artifacts
- test: 100% coverage
...
- feat: add a `direction` option to `IDBObjectStore`/`IDBIndex` `getAll`/`getAllKeys`, via a single `{query, count, direction}` options object (in addition to the existing `(query, count)` form); results are collected using the same low-level lookup a cursor's `continue()` uses internally, so ordering and uniqueness (`nextunique`/`prevunique`), including over `multiEntry` indexes, match cursor iteration exactly, while the whole operation still occupies a single, correctly-ordered slot in its transaction's request queue (rather than each step re-queuing behind later sibling requests, which could reorder its result relative to them)
- feat: add `IDBObjectStore`/`IDBIndex` `getAllRecords(options)` per the IndexedDB 3.0 draft, returning `IDBRecord`'s' with `{key, primaryKey, value}` for a given `{query, count, direction}`
